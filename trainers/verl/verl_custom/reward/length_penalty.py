...
class LengthPenalty:
    ...
    def get_repetition_reward(self, generations):
        ...
        args = [
            (generations[i], ngram_size, self.config.repetition_penalty)
            for i in range(bsz)
        ]
    ...
    def __call__(self, data: DataProto):
        ...
        with torch.no_grad():
            ...
            modulated_scores = torch.zeros_like(
                data.batch['responses'], dtype=torch.float32
            )

            if self.config.length_penalty_type == 'linear':
                modulated_scores = scores * (
                    1 - valid_response_length.unsqueeze(-1) / self.config.max_length
                )
                ...
                for i in range(bsz):
                    ...
                    if idx not in id2min:
                        ...
                        coef = 1.0 - (valid_response_length[i] - id2min[idx]) / (
                            self.config.max_length - id2min[idx]
                        )
                        length_coef.append(min(1.0, coef))

                modulated_scores[torch.arange(bsz), valid_response_length - 1] = (
                    torch.tensor(length_coef, dtype=torch.float32)
                )
                ...
                # Pre-compute masks
                is_binary_source = torch.tensor(
                    [
                        ds in self.config.binary_score_data_sources
                        for ds in data_sources
                    ],
                    device=instance_scores.device,
                )
                is_selected_source = torch.tensor(
                    [ds in self.config.selected_data_sources for ds in data_sources],
                    device=instance_scores.device,
                )
                ...
                if is_selected_source.any():
                    ...
                    long_gen_mask = (cur_scores <= 0.99) & (
                        valid_response_length + self.config.max_length_margin
                        > self.config.max_length
                    )
                    if long_gen_mask.any():
                        linear_penalty = (
                            max_values - min_values
                        ) / self.config.max_length_margin * (
                ...
                modulated_scores = torch.where(
                    response_mask.bool(),
                    modulated_scores,
                    torch.zeros_like(modulated_scores),
                )
                ...
                raise ValueError(
                    f'Length penalty type {self.config.length_penalty_type} not supported'
                )
