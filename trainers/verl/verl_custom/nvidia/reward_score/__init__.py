...
def _default_compute_score(data_source, solution_str, ground_truth, extra_info=None):
    # SWE-Bench fabric: only gsm8k and math scorers are active.
    # deepscaler, deepcoder, prime_math, prime_code, toolcall, ifeval, llm_judge removed.
    ...
    if data_source == 'openai/gsm8k':
        ...
        raise NotImplementedError(
            f'data_source={data_source!r} is not supported in the SWE-Bench fabric. '
            'Restore scorers from git history if needed.'
        )
...
def _remote_compute_score(
    data_source, solution_str, ground_truth, extra_info=None, session=None
):
    import requests
...
async def _remote_compute_score_async(
    data_source, solution_str, ground_truth, extra_info=None, session=None
):
    import aiohttp
