...
def apply_padding_mask(
    response_mask: torch.Tensor, is_padded: torch.Tensor, log_stats: bool = False
) -> torch.Tensor:
    """
    ...
    if isinstance(is_padded, (list, tuple)):
        is_padded = torch.tensor(
            is_padded, device=response_mask.device, dtype=torch.bool
        )
    ...
    if log_stats:
        ...
        if num_padded > 0:
            print(
                f'[PADDING MASK] Masking {num_padded}/{total_samples} samples ({num_padded / total_samples * 100:.1f}%) due to padding'
            )
