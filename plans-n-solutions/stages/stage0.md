# Stage 0 — Baseline sanity (v0.8 + vLLM 0.18)

**Status:** **DONE** — run #13, 20/20 steps, all 5 gates green. Commit `13f95697`. From this point forward, **all subsequent stages run against this upgraded stack** — do not fall back to v0.4 or vLLM 0.8.5.

---

## What this stage does

Reproduces a working 20-step GRPO run on the upgraded stack and locks a metrics baseline every later stage is compared against. No code changes during the run — observation only. The upgrade itself (v0.4 → v0.8.0.dev, vLLM 0.8.5 → 0.18) is part of this stage's problem log below.

| Component | Value |
|---|---|
| verl | v0.8.0.dev (`shamanez/verl` main, commit `910ba344`) at `/tmp/verl` |
| vLLM | 0.18 |
| PyTorch | 2.6+ (image) |
| Docker image | `verlai/verl:vllm018.dev1` |
| PyTorch alloc | `PYTORCH_ALLOC_CONF=expandable_segments:True` (fragmentation control on 40 GB) |
| `gpu_memory_utilization` override | `0.45` (leaves more VRAM for backward activations on A100-40GB) |
...
## How to run the baseline validation (self-contained)
...
```bash
bash scripts/_internal/s0_prorl.sh 2>&1 | tee /tmp/s0-prorl.log &
...
`s0_prorl.sh` sources `/home/ubuntu/.prorl_creds.env` and runs `poetry run python scripts/start_server.py --host 0.0.0.0 --port 8006 --max-init-workers 64 --max-run-workers 64 --timeout 1000`. It is the canonical ProRL launcher — do not reinvent it.
...
```bash
git add -A
git commit -m "Stage 0: baseline sanity on v0.8 + vLLM 0.18 stack

- All 26 compatibility issues resolved (see plans-n-solutions/stages/stage0.md)
...
- **v0.4 (old):** Workers are hybrid actors holding both FSDP training modules and vLLM inference engines. `AsyncActorRolloutRefWorker` has dispatch methods for both. vLLM engine lifecycle tied to training loop.
- **v0.8 (current):** Workers split into training actors and `RolloutReplica` inference actors. `AsyncActorRolloutRefWorker` only has training methods + `update_weights`. Inference handled by separate `vLLMHttpServer` actors via HTTP. The old dispatch pattern still exists in the codebase but no upstream worker uses it.
...
| `global_step ≥ 20` | **Pass** (run #13) |
| `actor/grad_norm` finite every step | Pass |
| `critic/rewards/mean` non-zero | Pass (0.375 → 0.5625 across early steps) |
| Advantage variance > 0 | Pass |
| `actor/kl` finite | Pass |

Run history (most recent first):

| Run | WandB | Outcome |
|---|---|---|
| #13 | (final) | All 5 gates green; 20/20 steps; commit `13f95697` |
...
- Runbook: this file + `scripts/_internal/s0_baseline_docker.sh` + `scripts/_internal/s0_prorl.sh` + `trainer_integration/verl/verl_custom/nvidia/scripts/run_proagent_qwn3_4B_instruct.sh`
- Next milestone: `plans-n-solutions/stages/stage1_playbook.md` (Stages 1 + 2)
