...
Remote vLLM pool for decoupled rollouts. Children run on a separate EC2 host (`vllm-instance`, 4 × 23 GiB GPUs); the trainer talks to them over public HTTP. Full plan + problem log: [`plans-n-solutions/stages/baseline.md`](../../plans-n-solutions/stages/baseline.md).
...
| File | Role |
|---|---|
| `_vllm_child.py` | vLLM 0.18 `AsyncLLMEngine`-backed `/generate` server. Accepts `{prompt_ids, …}` and returns `{response_ids, logprobs}`. Holds the 501 `/reload_weights` stub that Phase 1 (LoRA publish) replaces with `/reload_lora`. |
...
Env overrides: `REMOTE_HOST`, `REMOTE_DNS`, `REMOTE_POOL_DIR`, `REMOTE_PYTHON`, `MODEL`, `GPU_MEM_UTIL` (default 0.85), `MAX_MODEL_LEN` (default 17920).
...
No `--enforce-eager` — CUDA graphs are on, adds ~30–60 s one-time capture to `start` but gives ~30–50% throughput lift on Qwen3-4B. The 300 s health budget in `scripts/_internal/s1_remote_docker.sh` covers the capture.
